<script>
	...
	async function sendFrame() {
		...
		try {
			...
			const frame_data = canvasRef.toDataURL('image/jpeg', 0.7); // JPEG with 0.7 quality to reduce size
			...
			// Fire and forget - don't await response to avoid blocking UI too much
			fetch(WEBHOOK_URL, {
				method: 'POST',
				headers: {
	...
	async function initMediaPipe() {
		try {
			...
			if (videoSource) {
				...
				isMonitoring = true;
				startFrameStream(); // Start sending frames
		...
		<section class="camera-feed">
			<div class="video-wrapper">
				...
				<div class="overlay">
					...
					<div class="metric">
						<label>CLOUD</label>
						<span style="color: blue; font-weight: bold;">LIVE</span>
					</div>
				</div>
			</div>
		</section>
